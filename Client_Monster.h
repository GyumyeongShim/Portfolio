...
protected:
	MONSTER_STATE_TYPE		m_eFirstCategory = MONSTER_STATE_TYPE::IDLE; //일상,이동,죽음,피격,공격 중 1개
	MONSTER_IDLE_TYPE		m_eSecondCategory_IDLE = MONSTER_IDLE_TYPE::IDLE_END; //일상 중 어떤 애니를 사용할지
	MONSTER_MOVE_TYPE		m_eSecondCategory_MOVE = MONSTER_MOVE_TYPE::MOVE_END; //위와 동일, 이동관련
	MONSTER_ATK_TYPE		m_eSecondCategory_ATK = MONSTER_ATK_TYPE::ATK_END; //위와 동일, 공격관련
	MONSTER_HIT_TYPE		m_eSecondCategory_HIT = MONSTER_HIT_TYPE::HIT_END; //위와 동일, 피격관련
	MONSTER_CC_TYPE			m_eSecondCategory_CC = MONSTER_CC_TYPE::CC_END; //군중제어관련
	MONSTER_DEAD_TYPE		m_eSecondCategory_DEAD = MONSTER_DEAD_TYPE::DEAD_END; //죽음관련

	MONSTER_COLOR_TYPE		m_eMonsterColor = MONSTER_COLOR_TYPE::COLOR_NONE;
	//MONSTER_BULLET_TYPE		m_eBulletType = MONSTER_BULLET_TYPE::BULLET_NORMAL; //원거리 공격이 있는 친구만 갖는걸로 진행
	WEAPON_STATE			m_eWeaponState = WEAPON_STATE::WEAPON_None; //몬스터 무기소지 유무
	FBLR					m_eFBLR = FBLR::FBLR_END; //8방향체크

	_double					m_dTimeDelta = 0; //절대적으로 사용할 시간값
	_double					m_dAniPlayMul = 1; //애니재생배율
